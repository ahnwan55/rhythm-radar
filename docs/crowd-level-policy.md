...
- `activeTrackedMachineCount`는 최신 온라인 업데이트가 유지되며 현재 플레이 가능한 추적 대상 기체 수입니다.
- `totalMachineCount`, `trackedMachineCount`, `legacyMachineCount`는 매장 정보 또는 검수 정보로 표시할 수 있지만 체감 혼잡도 산정 입력으로 사용하지 않습니다.
- 구버전, 업데이트 중단 또는 오프라인 기체는 매장에 존재하더라도 계산에서 제외합니다.
...
최신 온라인 기체와 구버전 또는 업데이트 중단 기체가 한 점포에 함께 있는 경우:

- `activeTrackedMachineCount`에는 가동 중인 최신 온라인 추적 대상 기체만 포함합니다.
- 구버전 또는 업데이트 중단 기체는 `legacyMachineCount`에 기록할 수 있습니다.
- 오프라인 기체를 포함한 참고 정보는 `notes`에 표시할 수 있습니다.
- `legacyMachineCount`나 `notes`의 참고 기체 수는 조합표 적용이나 체감 혼잡도 완화에 사용하지 않습니다.

예를 들어 최신 온라인 기체 1대와 구버전 기체 2대가 있고 최신 기체의 상태가 `HIGH`라면, 계산은 1대 열의 `매우 혼잡`을 사용하며 구버전 2대 때문에 낮은 혼잡도로 바꾸지 않습니다.
