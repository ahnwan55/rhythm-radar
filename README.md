...
알파 1차 테스트는 운영자가 직접 검수하고 반복 확인할 수 있는 서울·부천권 5개 점포를 대상으로 시작합니다.
...
주안 CPU 게임랜드는 인천권 주요 리듬게임 점포로 확장 후보에 유지하되, 알파 1차에서는 운영자 직접 검수 가능 범위를 넘기므로 `DEFERRED` 상태로 보류합니다.
...
- 알파 1차 점포 5곳 중 노량진 어뮤즈타운, 한성대 우리게임장2, 부천 어택, 부천 P2존은 현장 조사 기준 기종 데이터를 반영했고, 나머지 1곳은 `PENDING_VERIFICATION` 상태입니다.
- 주안 CPU 게임랜드는 정적 데이터에 확장 후보로 남기되 `DEFERRED` 상태로 표시합니다.
...
- 상태: 문서화 중
- 기능 코드: 작성하지 않음
- 정적 데이터: 화면용 TypeScript 타입, 알파 1차 점포 5곳, 인천권 확장 후보 1곳, 노량진 어뮤즈타운, 한성대 우리게임장2, 부천 어택, 부천 P2존 현장 조사 데이터 작성
